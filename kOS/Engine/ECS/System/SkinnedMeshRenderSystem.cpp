    ...
    void SkinnedMeshRenderSystem::Update()
    {
        ...
        for (const EntityID id : entities) {
            ...
            //if (skinnedMesh->cachedSkinnedMeshGUID != skinnedMesh->skinnedMeshGUID)
            {
                ...
                {
                    // fix this
                    skeleton->m_CurrentTime += skeleton->GetTicksPerSecond() * m_ecs.m_GetDeltaTime();
                ...
                if (mesh)
                    m_graphicsManager.gm_PushSkinnedMeshData(SkinnedMeshData{ mesh, skeleton, PBRMaterial{diff,spec,rough,ao,norm}, transform->transformation, 0.f,id });
            }
            //else
